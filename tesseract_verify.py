...
def OCR_lmj(out, angle):
    ...
    # 保存图片
    dst_im.save("E://figures/hanzi/test.png")
    ...
    with open('E://figures/hanzi/recognize.txt', 'a') as f:
...
def recongnize_single_character(image_path):
    ...
    future_tasks = [executor.submit(OCR_lmj, out, angle) for angle in range(-50, 51, 1)]
    wait(future_tasks, return_when=ALL_COMPLETED)

    # 返回识别后的结果
    with open('E://figures/hanzi/recognize.txt', 'r') as f:
        words = f.readline()
...
def main():
    '''
    # 识别整个目录下的图片
    dir = 'E://figures/hanzi' # 修改目录为自己的目录

    correct_count = 0 # 识别正确的图片数量
    total_count = 0 # 图片总数
    for file in os.listdir(dir):
        if file.endswith('.gif'):
            # print(file)
            image_path = '%s/%s'%(dir,file) # 原先gif图片的路径
            image_path = change_picture_form(image_path) # 读取png格式的图片
            out = before_rorate(image_path) # 获取out值
            ...
            white_list = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmjopqrstuvwxyz0123456789【】，。；：‘’“”！？-[]{}<>:/\\;\'\"'
            recognizition = []
            for angle in range(-50, 51, 1): # 遍历旋转角度
                text = OCR_lmj(out, angle)
                if (len(text) == 1 and text not in white_list):
                    recognizition.append(text)

            # 输出答案及识别问题列表
            print((answer, recognizition))

            if answer in recognizition :
                correct_count += 1
    ...
    t1 = time.time()
    # 单张图片识别
    image_path = 'E://figures/hanzi/注.gif'

    answer = image_path.split('/')[-1].split('.')[0]
    print('答案：%s'%answer)

    word = recongnize_single_character(image_path)
    print('识别后的答案：%s'%word)
